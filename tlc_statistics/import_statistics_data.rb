...
system(%{psql $PG_CONNECTION_STRING -f create_statistics_tables.sql})
system(%{sort -u tlc_monthly_data.csv | psql $PG_CONNECTION_STRING -c "COPY tlc_monthly_reports FROM stdin CSV;"})
system(%{cat fhv_monthly_data.csv | psql $PG_CONNECTION_STRING -c "COPY fhv_monthly_reports FROM stdin CSV;"})
